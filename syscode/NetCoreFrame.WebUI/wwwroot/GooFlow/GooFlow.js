...
function GooFlow(bgDiv, property) {
	...
	if (this.$editable) {
		...
		this.$workArea.on("click", { inthis: this }, function (e) {
			...
			This.addNode(new Date().getTime(), { name: "node_" + This.$max, left: X, top: Y, type: This.$nowType, userlist: [] });
			This.$max++;
		});
		//划线或改线时用的绑定
...
GooFlow.prototype = {
	...
	addNode: function (id, json) {
		...
		if (json.type.indexOf(" round") < 0) {
			...
			this.$nodeDom[id] = $("<div class='GooFlow_item" + mark + "' id='" + id + "' style='top:" + json.top + "px;left:" + json.left + "px'><table cellspacing='1' style='width:" + (json.width - 2) + "px;height:" + (json.height - 2) + "px;'><tr><td class='ico'><i class='ico_" + json.type + "'></i></td><td class='icon_td'>" + json.name + "</td></tr></table><div style='display:none'><div class='rs_bottom'></div><div class='rs_right'></div><div class='rs_rb'></div><div class='rs_close'></div></div></div>");
		}
		//圆点
		else {
	...
	exportData: function () {
		...
		for (var k1 in ret.nodes) {
			var childnode = {}; //节点
			var userlist = {}; //审批人
			childnode.id = k1.toString();
			childnode.name = ret.nodes[k1].name;
			childnode.left = ret.nodes[k1].left;
			childnode.top = ret.nodes[k1].top;
			childnode.type = ret.nodes[k1].type;
			childnode.width = ret.nodes[k1].width;
			childnode.height = ret.nodes[k1].height;
			childnode.alt = ret.nodes[k1].alt;
			childnode.userlist = [];
			for (var userMol in ret.nodes[k1].userlist) {
				childnode.userlist.push(ret.nodes[k1].userlist[userMol]);
			}
			$nodesNew.push(childnode);
		...
		for (var k2 in ret.lines) {
			var childnode = {}; //节点
			var userlist = {}; //审批人
			childnode.id = k2.toString();
			childnode.type = ret.lines[k2].type;
			childnode.from = ret.lines[k2].from;
			childnode.to = ret.lines[k2].to;
			childnode.name = ret.lines[k2].name;
			if (!ret.lines[k2].marked) {
				delete ret.lines[k2]["marked"];
			}
			$linesNew.push(childnode);
		}
		var ret2 = {
			title: this.$title,
			nodes: $nodesNew,
